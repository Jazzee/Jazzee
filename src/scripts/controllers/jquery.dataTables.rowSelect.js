//var ROW_CSS_CLASS = "row_selected";
var ROW_CSS_CLASS = "DTTT_selected";
/*
...
(function ($) {
    ...
    $.fn.rowSelect = function (options) {
	    ...
	    console.log("in rowselect, making rows clickable");

	    //	    $("tbody", oTable).find("tr").each(function(idx, elem){
	    $(oTable.fnGetNodes()).each(function(idx, elem){

		    if(!$(elem).attr("setup-selection")){
			console.log("assigning click handler!");
			$(elem).on("click", function(evt){
				console.log("clicked: "+this);
				if(evt.shiftKey){
				    console.log("last clicked? "+oTable.lastClicked);
				    if(oTable.lastClicked){
					var from = oTable.lastClicked.rowIndex;
					var to = this.rowIndex;
					console.log("last idx: "+from+" -> "+to);
					var larger = (from < to) ? to : from;
					var smaller = (from < to) ? from : to;

					for(var i=smaller; i<=larger; i++){
					    if(i == from){
						// leave the original row selected
					    }else{
						$(this).closest("table.dataTable").find("tr").eq(i).toggleClass(ROW_CSS_CLASS);
					    }
					}
					oTable.lastClicked = this;
				    }
				}else{
				    $(this).toggleClass(ROW_CSS_CLASS);
				    oTable.lastClicked = this;
				}

			    });
			$(elem).attr("setup-selection", true);
		    }
		});
	    /*
