...
<div class='answers'>
  <table class='answer'>
    <thead>
      <tr>
        <th>Recommender</th>
        <?php foreach($page->getPage()->getChildren()->first()->getChildren() as $branch){?>
           <?php foreach($branch->getElements() as $element){?> 
      ...
      <tr id='answer<?php print $answer->getId() ?>'>
        ...
        <?php foreach($page->getPage()->getChildren()->first()->getChildren() as $branch){?>
          <?php foreach($branch->getElements() as $element){
            ...
            <td><?php print $element->getJazzeeElement()->displayValue(($answer->getChildren()->first()?$answer->getChildren()->first()->getChildren()->first():$answer)); ?></td>
          <?php }?>
            <?php }?>
