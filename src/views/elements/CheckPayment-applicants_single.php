      ...
      case \Jazzee\Entity\Payment::PENDING:?>
        ...
        <h5>Directions to applicant</h5>
          <?php
            ...
            print str_ireplace($search, $replace, $text);
          ?>
        <div class='tools'>
          <?php if($this->controller->checkIsAllowed('applicants_single', 'settlePayment')){ ?>
            <a href='<?php print $this->path('applicants/single/' . $answer->getApplicant()->getId() . '/settlePayment/' . $answer->getId());?>' class='actionForm'>Clear Check</a>
          <?php } ?>
          <?php if($this->controller->checkIsAllowed('applicants_single', 'rejectPayment')){ ?>
            <a href='<?php print $this->path('applicants/single/' . $answer->getApplicant()->getId() . '/rejectPayment/' . $answer->getId());?>' class='actionForm'>Void Payment</a>   
          <?php } ?>
        </div>
      <?php break;
      case \Jazzee\Entity\Payment::SETTLED:?>
        ...
        <div class='tools'>
          <?php if($this->controller->checkIsAllowed('applicants_single', 'refundPayment')){ ?>
            <a href='<?php print $this->path('applicants/single/' . $answer->getApplicant()->getId() . '/refundPayment/' . $answer->getId());?>' class='actionForm'>Record a refund payment</a>   
          <?php } ?>
        </div>
      <?php break;
