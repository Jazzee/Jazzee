...
?>
  ...
  if(!empty($currentAnswerID) or is_null($page->getMax()) or count($page->getJazzeePage()->getAnswers()) < $page->getMax()){?>
    <div id='leadingText'><?php print $page->getLeadingText()?></div>
      <?php if($confirm){ ?>
      <?php $picklist = $this->controller->getVar('picklist');?>
        <script type = "text/javascript">
          $(document).ready(function(){
            var form = $('<form>')
              .attr('id', 'formPicklist')
              .attr('method', 'post')
              .attr('action','<?php print $this->controller->getActionPath();?>/do/pickAddress<?php if(!empty($currentAnswerID)) print "/{$currentAnswerID}";?>');
            ...
            $('body').append(form);
            var addresses = [];
            <?php foreach($picklist->atItems as $address){?>
                addresses.push({address:"<?php print $address->Picklist; ?>", moniker:"<?php print $address->Moniker; ?>"});
            <?php } ?>
        ...
        <fieldset>
          ...
          <div id='pickAddresses'>
            <noscript>
              <form name = "formPicklist" id = "formPicklist" method = "post" action = "<?php print $this->controller->getActionPath();?>/do/pickAddress<?php if(!empty($currentAnswerID)) print "/{$currentAnswerID}";?>">
