  ...
  <?php foreach($pendingPayments as $payment){?>
    <tr>
      ...
      <?php 
        ...
        if($payment->getAnswer()->getApplicant()->isLocked()) $tags[] = 'Locked';
        if($payment->getAnswer()->getApplicant()->hasPaid()) $tags[] = 'Paid';
        if($payment->getAnswer()->getApplicant()->getDecision() and $payment->getAnswer()->getApplicant()->getDecision()->getAcceptOffer()) $tags[] = 'Accepted';
        if($payment->getAnswer()->getApplicant()->getDecision() and $payment->getAnswer()->getApplicant()->getDecision()->getFinalAdmit()) $tags[] = 'Admitted';
        if($payment->getAnswer()->getApplicant()->getDecision() and $payment->getAnswer()->getApplicant()->getDecision()->getDeclineOffer()) $tags[] = 'Declined';
        if($payment->getAnswer()->getApplicant()->getDecision() and $payment->getAnswer()->getApplicant()->getDecision()->getFinalDeny()) $tags[] = 'Denied';
        asort($tags);
        $tags = implode(', ', $tags);
      ?>
